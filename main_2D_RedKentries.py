...
K00, K11, K33 = 2556.12, 5.1035e07, 4.79732e07
# K00, K11, K33 = 2.5e2, 2.5e7, 2.5e7 # Values for 82 nelx length
constraints = numpy.array(
    [K00, K11, K33, K00, K11, K33]
)  # Twice the same to generate the equality constraint from the negative and postivive side
...
problem = MinMassRedKentries2(
    ...
    domain_lens,
    joint_locs,
)  # Only pass Kreq if MinMassRedKentries problem
...
print(
    [
        (Kgr[0, 0] - K00r) * 100 / K00r,
        (Kgr[1, 1] - K11r) * 100 / K11r,
        (Kgr[3, 3] - K33r) * 100 / K33r,
    ]
)

# Component Mass
rho_material = 2700  # kg/m^3
volume_fraction = solver.opt.last_optimum_value() / (
    nelx * nely * nelz
)  # Sum of densities divided by number of elements
